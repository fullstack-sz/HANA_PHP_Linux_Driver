...
ss_error SS_ERRORS[] = {
          
    {
        ...
        { IMSSP, (SQLCHAR*)"Invalid option %1!s! was passed to hdb_connect.", -1, true }
    },
    ...
    
    {
        ...
        { IMSSP, (SQLCHAR*)"An invalid fetch type was specified. HDB_FETCH_NUMERIC, HDB_FETCH_ARRAY and HDB_FETCH_BOTH are acceptable values.", -10, false }
    },
    ...
    {
        SS_HDB_ERROR_ALREADY_IN_TXN,
        { IMSSP, (SQLCHAR*)"Cannot begin a transaction until the current transaction has been completed by calling either "
          "hdb_commit or hdb_rollback.", -12, false }
    },  
    
    {
        SS_HDB_ERROR_NOT_IN_TXN,
        { IMSSP, (SQLCHAR*)"A transaction must be started by calling hdb_begin_transaction before calling hdb_commit or "
          "hdb_rollback.", -13, false }
    },  
      ...
      
    {
        ...
        { IMSSP, (SQLCHAR*)"An invalid direction for parameter %1!d! was specified. HDB_PARAM_IN, HDB_PARAM_OUT, and "
          "HDB_PARAM_INOUT are valid values.", -15, true }
    },
...

    {
        ...
        { IMSSP, (SQLCHAR*)"A row must be retrieved with hdb_fetch before retrieving data with hdb_get_field.", -18, false }
    },    

    {
        HDB_ERROR_FIELD_INDEX_ERROR,
        { IMSSP, (SQLCHAR*)"Fields within a row must be accessed in ascending order. "
          "The hdb_get_field function cannot retrieve field %1!d! because its index is less "
    ...
    // no equivalent to HDB_ERROR_SERVER_INFO in 2.0 so -21 is skipped
    ...
    
    {
        ...
        { IMSSP, (SQLCHAR*)"A statement must be prepared with hdb_prepare before calling hdb_execute.", -23, false }
    },   
    ...
    // these three errors are returned for invalid options, so they are given the same number for compatibility with 1.1
    {
        ...
        { IMSSP, (SQLCHAR*) "Invalid value %1!s! specified for option HDB_QUERY_TIMEOUT.", -33, true }
    },
        ...
        
    {
        ...
        { IMSSP, (SQLCHAR*)"The type of output parameter %1!d! does not match the type specified by the HDB_PHPTYPE_* constant."
           " For output parameters, the type of the variable's current value must match the HDB_PHPTYPE_* constant, or be NULL. "
           "If the type is NULL, the PHP type of the output parameter is inferred from the HDB_SQLTYPE_* constant.", -34, true }
    },
    ...
    {  
        ...
        { IMSSP, (SQLCHAR*)"The scroll type passed to hdb_fetch, hdb_fetch_array, or hdb_fetch_object was not valid.  "
          "Please use one of the HDB_SCROLL constants.", -53, false }
    },
    ...
    },
    {
        ...
        "Output or bidirectional variable parameters (HDB_PARAM_OUT and HDB_PARAM_INOUT) passed to hdb_prepare or hdb_query should be passed by reference, not by value."
        , -61, true }
    },
	{
	...
	},
    {
        ...
        { IMSSP, (SQLCHAR*)"Must specify the SQL type for each parameter in a parameterized query when using hdb_query in a column encryption enabled connection.", -63, false }
    },

    // internal warning definitions
    {
        SS_HDB_WARNING_FIELD_NAME_EMPTY,
        { SSPWARN, (SQLCHAR*)"An empty field name was skipped by hdb_fetch_object.", -100, false }
    },
    {
...
void ss_hdb_log( _In_ unsigned int severity TSRMLS_DC, _In_opt_ const char* msg, _In_opt_ va_list* print_args )
{
    if(( severity & HDB_G( log_severity )) && ( HDB_G( current_subsystem ) & HDB_G( log_subsystems ))) {
        ...
        if( rc == 0 ) {
            HDB_STATIC_ASSERT( sizeof( INTERNAL_FORMAT_ERROR ) < sizeof( log_msg ));
...
//
// Returns extended error and/or warning information about the last hdb
// operation performed.
//
// The hdb_errors function can return error and/or warning information by
...
// 
//  HDB_ERR_ALL
//      Errors and warnings generated on the last hdb function call are returned.
//  HDB_ERR_ERRORS
//      Errors generated on the last hdb function call are returned.
//  HDB_ERR_WARNINGS
//      Warnings generated on the last hdb function call are returned.
//
// If no parameter value is supplied, HDB_ERR_ALL is the default
//
// Return Value
...
{
    HDB_UNUSED( execute_data );

    zend_long flags = HDB_ERR_ALL;
...
// hdb_configure( string $setting, mixed $value )
//
...
//
// Return Value
// If hdb_configure is called with an unsupported setting or value, the
...
PHP_FUNCTION( hdb_configure )
{
    ...
    try {
        ...
        HDB_ASSERT( option[option_len] == '\0', "hdb_configure: option was not null terminated." );
        if( !stricmp( option, INI_WARNINGS_RETURN_AS_ERRORS )) {

            HDB_G( warnings_return_as_errors ) = zend_is_true( value_z ) ? true : false;
        ...
        // LogSeverity
            ...
            CHECK_CUSTOM_ERROR(( Z_TYPE_P( value_z ) != IS_LONG ), error_ctx, SS_HDB_ERROR_INVALID_FUNCTION_PARAMETER, _FN_ ) {
            ...
            HDB_G( log_severity ) = static_cast<logging_severity>( severity_mask );
        ...
        // LogSubsystems
            ...
            HDB_G( log_subsystems ) = static_cast<logging_subsystems>( subsystem_mask );
            ...
            CHECK_CUSTOM_ERROR(( Z_TYPE_P( value_z ) != IS_LONG ), error_ctx, HDB_ERROR_INVALID_BUFFER_LIMIT, _FN_ ) {
            ...
            CHECK_CUSTOM_ERROR( buffered_query_limit <= 0, error_ctx, HDB_ERROR_INVALID_BUFFER_LIMIT, _FN_ ) {

                throw ss::SSException();
            }

            HDB_G( buffered_query_limit ) = buffered_query_limit;
        ...
        DIE( "hdb_configure: Unknown exception caught." );
    }
}


// hdb_get_config( string $setting )
// 
...
//  list of configurable settings, see hdb_configure.
//
// Return Value
...
// really determine if an error occurred, call hdb_errors.
...
PHP_FUNCTION( hdb_get_config )
{
    ...
    try {
        ...
        HDB_ASSERT( option != NULL, "hdb_get_config: option was null." );
        if( !stricmp( option, INI_WARNINGS_RETURN_AS_ERRORS )) {

            ZVAL_BOOL( return_value, HDB_G( warnings_return_as_errors ));
            return;
        }
        else if( !stricmp( option, INI_LOG_SEVERITY )) {

            ZVAL_LONG( return_value, HDB_G( log_severity ));
            return;
        }
        else if( !stricmp( option, INI_LOG_SUBSYSTEMS )) {

            ZVAL_LONG( return_value, HDB_G( log_subsystems ));
            return;
        }
        else if( !stricmp( option, INI_BUFFERED_QUERY_LIMIT )) {

            ZVAL_LONG( return_value, HDB_G( buffered_query_limit ));
            return;
        }
        else {
        ...
        DIE( "hdb_get_config: Unknown exception caught." );
    }
}
...
hdb_error_const* get_error_message( _In_ unsigned int hdb_error_code ) {
	...
	hdb_error_const *error_message = NULL;

	int zr = ( error_message = reinterpret_cast< hdb_error_const* >( zend_hash_index_find_ptr( g_ss_errors_ht, hdb_error_code ))) != NULL ? SUCCESS : FAILURE;
	if ( zr == FAILURE ) {
		DIE( "get_error_message: zend_hash_index_find returned failure for hdb_error_code = %1!d!", hdb_error_code );
	}

	HDB_ASSERT( error_message != NULL, "get_error_message: error_message was null" );
                         ...
                         _In_ bool warning TSRMLS_DC )
{
    ...
    zval temp; 
	ZVAL_UNDEF(&temp);
	core::hdb_zval_stringl( &temp, reinterpret_cast<char*>( error->sqlstate ), SQL_SQLSTATE_SIZE );
    //TODO: reference?
	Z_TRY_ADDREF_P( &temp );
                                 ...
                                 _In_ unsigned int hdb_error_code, _In_ bool warning, _In_opt_ va_list* print_args TSRMLS_DC )
{
    ...
    if( warning ) {
        ...
        if( HDB_G( warnings_return_as_errors ) ) {
...
bool ignore_warning( _In_ char* sql_state, _In_ int native_code TSRMLS_DC )
{
	...
	ZEND_HASH_FOREACH_KEY_PTR( g_ss_warnings_to_ignore_ht, index, key, error_temp ) {
		hdb_error* error = static_cast<hdb_error*>( error_temp );
		if (NULL == error) {
			return false;
		}
