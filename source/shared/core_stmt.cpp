...
// File: core_stmt.cpp
//
// Contents: Core routines that use statement handles shared between hdb and pdo_hdb
//
//---------------------------------------------------------------------------------------------------------------------------------
...
struct field_cache {
    ...
    field_cache( _In_reads_bytes_opt_(field_len) void* field_value, _In_ SQLLEN field_len, _In_ hdb_phptype t )
        : type( t )
    {
        // if the value is NULL, then just record a NULL pointer
        if( field_value != NULL ) {
            value = hdb_malloc( field_len );
...
void hdb_stream_dtor( _Inout_ zval* data );
...
hdb_stmt::hdb_stmt( _In_ hdb_conn* c, _In_ SQLHANDLE handle, _In_ error_callback e, _In_opt_ void* drv TSRMLS_DC ) :
    ...
    buffered_query_limit( hdb_buffered_result_set::BUFFERED_QUERY_LIMIT_INVALID ),
    ...
    current_stream( NULL, HDB_ENCODING_DEFAULT ),
...
hdb_stmt::~hdb_stmt( void )
{
    ...
    if( current_results ) {
        current_results->~hdb_result_set();
...
void hdb_stmt::new_result_set( TSRMLS_D )
{
    ...
    if( cursor_type == HDB_CURSOR_BUFFERED ) {
        ...
        current_results = new (hdb_malloc( sizeof( hdb_odbc_result_set ))) hdb_odbc_result_set( this );
    }
}

// core_hdb_create_stmt
                                      ...
                                      _In_opt_ const stmt_option valid_stmt_opts[], _In_ error_callback const err, _In_opt_ void* driver TSRMLS_DC )
{
    ...
    try {
        ...
        // handle has been set in the constructor of ss_hdb_stmt, so we set it to NULL to prevent a double free
        // in the catch block below.
        stmt_h = SQL_NULL_HANDLE;

        // process the options array given to core_hdb_prepare.
    ...
    catch( core::CoreException& )
    {
        ...
        DIE( "core_hdb_allocate_stmt: Unknown exception caught." );
    }
...
// column_size    - length of the field on the server (HDB_UKNOWN_SIZE means not known, so infer defaults)
                             ...
                             _Inout_ SQLSMALLINT decimal_digits TSRMLS_DC )
{
    ...
    HDB_ASSERT( direction == SQL_PARAM_INPUT || direction == SQL_PARAM_OUTPUT || direction == SQL_PARAM_INPUT_OUTPUT,
                   "core_hdb_bind_param: Invalid parameter direction." );
    HDB_ASSERT( direction == SQL_PARAM_INPUT || php_out_type != HDB_PHPTYPE_INVALID,
                   "core_hdb_bind_param: php_out_type not set before calling core_hdb_bind_param." );

    try {

    // check is only < because params are 0 based
    CHECK_CUSTOM_ERROR( param_num >= SQL_SERVER_MAX_PARAMS, stmt, HDB_ERROR_MAX_PARAMS_EXCEEDED, param_num + 1 ){
    ...
    if( direction == SQL_PARAM_INPUT_OUTPUT ) {
        ...
        switch( php_out_type ){
            ...
            case HDB_PHPTYPE_STREAM:
                HDB_ASSERT( false, "Invalid type for an output parameter." );
                break;
            default:
                HDB_ASSERT( false, "Unknown HDB_PHPTYPE_* constant given." );
                break;
        }
        CHECK_CUSTOM_ERROR( !match, stmt, HDB_ERROR_INPUT_OUTPUT_PARAM_TYPE_MATCH, param_num + 1 ){
            throw core::CoreException();
        }
    }
    ...
    CHECK_CUSTOM_ERROR( direction != SQL_PARAM_INPUT && ( sql_type == SQL_LONGVARCHAR
                        ...
                        stmt, HDB_ERROR_OUTPUT_PARAM_TYPES_NOT_SUPPORTED ){
        throw core::CoreException();
    }
    ...
    if( direction == SQL_PARAM_OUTPUT ){
        switch( php_out_type ) {
            ...
            case HDB_PHPTYPE_STREAM:
                ...
                break;
            default:
                HDB_ASSERT( false, "Uknown HDB_PHPTYPE_* constant given" );
                break;
        }
    }

    HDB_ASSERT(( Z_TYPE_P( param_z ) != IS_STRING && Z_TYPE_P( param_z ) != IS_RESOURCE ) ||
                  ( encoding == HDB_ENCODING_SYSTEM || encoding == HDB_ENCODING_UTF8 ||
                    encoding == HDB_ENCODING_BINARY ), "core_hdb_bind_param: invalid encoding" );

    if( stmt->conn->ce_option.enabled && ( sql_type == SQL_UNKNOWN_TYPE || column_size == HDB_UNKNOWN_SIZE )){
        // use the meta data only if the user has not specified the sql type or column size
        HDB_ASSERT( param_num < stmt->param_descriptions.size(), "Invalid param_num passed in core_hdb_bind_param!" );
    ...
    switch( Z_TYPE_P( param_z )){
        ...
        case IS_STRING:
            {
                ...
                if( direction == SQL_PARAM_INPUT && encoding == CP_UTF8 ){
                    ...
                    CHECK_CUSTOM_ERROR( !converted, stmt, HDB_ERROR_INPUT_PARAM_ENCODING_TRANSLATE,
                ...
                if( direction != SQL_PARAM_INPUT ){
                    ...
                    if( ZSTR_IS_INTERNED( Z_STR_P( param_z ))){
                        core::hdb_zval_stringl( param_z, static_cast<const char*>(buffer), buffer_len );
                        buffer = Z_STRVAL_P( param_z );
                        buffer_len = Z_STRLEN_P( param_z );
                    }
        ...
        case IS_OBJECT:
        {
            ...
            CHECK_CUSTOM_ERROR( !valid_class_name_found, stmt, HDB_ERROR_INVALID_PARAMETER_PHPTYPE, param_num + 1 ){
                throw core::CoreException();
            }
            ...
            /*if( sql_type == SQL_SS_TIMESTAMPOFFSET ){
				core::hdb_zval_stringl( &format_z, const_cast<char*>( DateTime::DATETIMEOFFSET_FORMAT ),
                              DateTime::DATETIMEOFFSET_FORMAT_LEN );
            }
            else*/ if( sql_type == SQL_TYPE_DATE ){
				core::hdb_zval_stringl( &format_z, const_cast<char*>( DateTime::DATE_FORMAT ), DateTime::DATE_FORMAT_LEN );
            }
            else{
				core::hdb_zval_stringl( &format_z, const_cast<char*>( DateTime::DATETIME_FORMAT ), DateTime::DATETIME_FORMAT_LEN );
            }
            // call the DateTime::format member function to convert the object to a string that SQL Server understands
			core::hdb_zval_stringl( &function_z, "format", sizeof( "format" ) - 1 );
            params[0] = format_z;
        ...
        case IS_ARRAY:
            THROW_CORE_ERROR( stmt, HDB_ERROR_INVALID_PARAMETER_PHPTYPE, param_num + 1 );
            break;
       default:
            DIE( "core_hdb_bind_param: Unsupported PHP type.  Only string, float, int, and streams (resource) are supported. "
...
// Parameters:
// stmt - the core hdb_stmt structure that contains the ODBC handle
// Return:
    ...
    if( sql ) {
        ...
        if( sql_len == 0 || ( sql[0] == '\0' && sql_len == 1 )) {
            ...
            wsql_string = utf16_string_from_mbcs_string( encoding, reinterpret_cast<const char*>( sql ),
                                                         sql_len, &wsql_len );
            CHECK_CUSTOM_ERROR( wsql_string == 0, stmt, HDB_ERROR_QUERY_STRING_ENCODING_TRANSLATE,
...
// Parameters:
// stmt              - the hdb_stmt of the cursor
...
bool core_hdb_fetch( _Inout_ hdb_stmt* stmt, _In_ SQLSMALLINT fetch_orientation, _In_ SQLULEN fetch_offset TSRMLS_DC )
{
    ...
    HDB_ASSERT( fetch_orientation >= SQL_FETCH_NEXT || fetch_orientation <= SQL_FETCH_RELATIVE,
                   "core_hdb_fetch: Invalid value provided for fetch_orientation parameter." );
    ...
    try {
        ...
        CHECK_CUSTOM_ERROR( stmt->past_fetch_end, stmt, HDB_ERROR_FETCH_PAST_END ) {
            throw core::CoreException();
        }
        // First time only
        ...
        DIE( "core_hdb_fetch: Unexpected exception occurred." );
    }
...
field_meta_data* core_hdb_field_metadata( _Inout_ hdb_stmt* stmt, _In_ SQLSMALLINT colno TSRMLS_DC )
{
    ...
    hdb_malloc_auto_ptr<field_meta_data> meta_data;
    hdb_malloc_auto_ptr<SQLWCHAR> field_name_temp;
    SQLSMALLINT field_len_temp = 0;
    SQLLEN field_name_len = 0;

    meta_data = new ( hdb_malloc( sizeof( field_meta_data ))) field_meta_data();
    field_name_temp = static_cast<SQLWCHAR*>( hdb_malloc( ( SS_MAXCOLNAMELEN + 1 ) * sizeof( SQLWCHAR ) ));
    HDB_ENCODING encoding = ( (stmt->encoding() == HDB_ENCODING_DEFAULT ) ? stmt->conn->encoding() : stmt->encoding());
	try{
...
// Parameters:
// stmt                 - the hdb_stmt from which to retrieve the column
// field_index          - 0 based index for the column to retrieve
// hdb_php_type_in   - hdb_php_type structure that tells what format to return the data in
	...
	try {
		...
		if (NULL != ( cached = static_cast<field_cache*>( zend_hash_index_find_ptr( Z_ARRVAL( stmt->field_cache ), static_cast<zend_ulong>( field_index ))))) {
			...
			if( cached->value == NULL ) {
				field_value = NULL;
				*field_len = 0;
				if( hdb_php_type_out ) { *hdb_php_type_out = HDB_PHPTYPE_NULL; }
			}
			else {

				field_value = hdb_malloc( cached->len, sizeof( char ), 1 );
				memcpy_s( field_value, ( cached->len * sizeof( char )), cached->value, cached->len );
				if( cached->type.typeinfo.type == HDB_PHPTYPE_STRING) {
					// prevent the 'string not null terminated' warning
					reinterpret_cast<char*>( field_value )[ cached->len ] = '\0';
				}
				*field_len = cached->len;
				if( hdb_php_type_out) { *hdb_php_type_out = static_cast<HDB_PHPTYPE>(cached->type.typeinfo.type); }
			}
			return;
		}

		hdb_phptype hdb_php_type = hdb_php_type_in;
		...
		CHECK_CUSTOM_ERROR( !is_valid_hdb_phptype( hdb_php_type ), stmt, HDB_ERROR_INVALID_TYPE ) {
			throw core::CoreException();
		}

		if( hdb_php_type_out != NULL )
...
// core_hdb_has_any_result
// return if any result set or rows affected message is waiting
// to be consumed and moved over by hdb_next_result.
// Parameters:
...
void core_hdb_next_result( _Inout_ hdb_stmt* stmt TSRMLS_DC, _In_ bool finalize_output_params, _In_ bool throw_on_errors )
{
    try {
        ...
        CHECK_CUSTOM_ERROR( stmt->past_next_result_end, stmt, HDB_ERROR_NEXT_RESULT_PAST_END ) {
            throw core::CoreException();
        }
...
void core_hdb_set_scrollable( _Inout_ hdb_stmt* stmt, _In_ unsigned long cursor_type TSRMLS_DC )
{
    try {
        ...
        switch( cursor_type ) {
            ...
            default:
                THROW_CORE_ERROR( stmt, HDB_ERROR_INVALID_OPTION_SCROLLABLE );
                break;
        }
        ...
        if( read > 0 ) {
            ...
            if( stmt->current_stream.encoding == CP_UTF8 ) {
				...
				if( wsize == 0 && last_error_code == ERROR_NO_UNICODE_TRANSLATION ) {
                    ...
                    CHECK_CUSTOM_ERROR( new_read != need_to_read, stmt, HDB_ERROR_INPUT_STREAM_ENCODING_TRANSLATE, get_last_error_message( ERROR_NO_UNICODE_TRANSLATION )) {
                        throw core::CoreException();
                    }
                    ...
                    CHECK_CUSTOM_ERROR( wsize == 0, stmt, HDB_ERROR_INPUT_STREAM_ENCODING_TRANSLATE, get_last_error_message( ERROR_NO_UNICODE_TRANSLATION )) {
                        throw core::CoreException();
                    }
                }
...
void close_active_stream( _Inout_ hdb_stmt* stmt TSRMLS_DC )
{
    ...
    HDB_ASSERT(( stream != NULL ), "close_active_stream: Unknown resource type as our active stream." );

    php_stream_close( stream ); // this will NULL out the active stream in the statement.  We don't check for errors here.

    HDB_ASSERT( Z_TYPE( stmt->active_stream ) == IS_UNDEF, "close_active_stream: Active stream not closed." );
...
size_t calc_utf8_missing( _Inout_ hdb_stmt* stmt, _In_reads_(buffer_end) const char* buffer, _In_ size_t buffer_end TSRMLS_DC )
{
    ...
    switch( *last_char & UTF8_NBYTESEQ_MASK ) {
        ...
        case UTF8_4BYTESEQ_TAG:
            ...
            break;
        default:
            THROW_CORE_ERROR( stmt, HDB_ERROR_INPUT_STREAM_ENCODING_TRANSLATE,
                            ...
                            hdb_php_type, _Inout_updates_bytes_(*field_len) void*& field_value, _Inout_ SQLLEN* field_len TSRMLS_DC )
{
    try {
        ...
        CHECK_CUSTOM_ERROR( stmt->last_field_index > field_index, stmt, HDB_ERROR_FIELD_INDEX_ERROR, field_index,
                            stmt->last_field_index ) {
            throw core::CoreException();
        }

        switch( hdb_php_type.typeinfo.type ) {

        case HDB_PHPTYPE_INT:
        {
            hdb_malloc_auto_ptr<long> field_value_temp;
            field_value_temp = static_cast<long*>( hdb_malloc( sizeof( long )));
            *field_value_temp = 0;
        ...
        case HDB_PHPTYPE_FLOAT:
        {
            hdb_malloc_auto_ptr<double> field_value_temp;
        ...
        case HDB_PHPTYPE_DATETIME:
        {
            ...
            return_value_z = ( zval * )hdb_malloc( sizeof( zval ));
            ...
            core::hdb_zval_stringl( &field_value_temp_z, field_value_temp, *field_len );
            core::hdb_zval_stringl( &function_z, "date_create", sizeof("date_create") - 1 );
        ...
        case HDB_PHPTYPE_STREAM:
        {
            ...
            CHECK_CUSTOM_ERROR( !is_streamable_type( sql_type ), stmt, HDB_ERROR_STREAMABLE_TYPES_ONLY ) {
                throw core::CoreException();
            }

            stream = php_stream_open_wrapper( "hdb://sqlncli10", "r", 0, NULL );

            CHECK_CUSTOM_ERROR( !stream, stmt, HDB_ERROR_STREAM_CREATE ) {
                throw core::CoreException();
            }

            ss = static_cast<hdb_stream*>( stream->abstract );
            ss->stmt = stmt;
            ss->field_index = field_index;
            ss->sql_type = static_cast<SQLUSMALLINT>( sql_type );
            ss->encoding = static_cast<HDB_ENCODING>( hdb_php_type.typeinfo.encoding );
        ...
        default:
            DIE( "core_get_field_common: Unexpected hdb_phptype provided" );
            break;
        }
...
bool check_for_next_stream_parameter( _Inout_ hdb_stmt* stmt TSRMLS_DC )
{
    ...
    hdb_stream* stream_encoding = NULL;
    zval* param_z = NULL;
    ...
    // pull out the hdb_encoding struct
    stream_encoding = reinterpret_cast<hdb_stream*>(zend_hash_index_find_ptr(streams_ht, stream_index));
    HDB_ASSERT(stream_encoding != NULL, "Stream parameter does not exist");  // if the index isn't in the hash, that's a serious error

    param_z = stream_encoding->stream_z;

    // make the next stream current
    stmt->current_stream = hdb_stream( param_z, stream_encoding->encoding );
    stmt->current_stream_read = 0;
...
{
    HDB_ASSERT( input_param_z == converted_param_z || Z_TYPE_P( converted_param_z ) == IS_NULL,
    ...
    if( buffer_len == 0 ) {
		core::hdb_zval_stringl( converted_param_z, "", 0 );
        return true;
    }
    ...
    hdb_malloc_auto_ptr<SQLWCHAR> wbuffer;
    wbuffer = reinterpret_cast<SQLWCHAR*>( hdb_malloc( (wchar_size + 1) * sizeof( SQLWCHAR ) ));
    ...
    core::hdb_zval_stringl( converted_param_z, reinterpret_cast<char*>( wbuffer.get() ), wchar_size * sizeof( SQLWCHAR ) );
    hdb_free(wbuffer);
    wbuffer.transferred();
        ...
        case IS_RESOURCE:
            switch( encoding ) {
                ...
                case CP_UTF8:
                    ...
                    break;
                default:
                    THROW_CORE_ERROR( stmt, HDB_ERROR_INVALID_PARAMETER_ENCODING, paramno );
                    break;
            }
            break;
        // it is assumed that an object is a DateTime since it's the only thing we support.
        // verification that it's a real DateTime object occurs in core_hdb_bind_param.
        // we convert the DateTime to a string before sending it to the server.
        case IS_OBJECT:
            sql_c_type = SQL_C_CHAR;
            break;
        default:
            THROW_CORE_ERROR( stmt, HDB_ERROR_INVALID_PARAMETER_PHPTYPE, paramno );
            break;
    }
                       ...
                       _Out_ SQLSMALLINT& sql_type TSRMLS_DC )
{
    ...
    switch( php_type ) {
        ...
        case IS_OBJECT:
            ...
            // as a HDB_SQLTYPE_DATETIME, otherwise it should be HDB_SQLTYPE_TIMESTAMPOFFSET
                                 ...
                                 _Out_ SQLULEN& column_size, _Out_ SQLSMALLINT& decimal_digits TSRMLS_DC )
{
    ...
    switch( php_type ) {
        ...
        case IS_STRING:
        {
            size_t char_size = (encoding == HDB_ENCODING_UTF8 ) ? sizeof( SQLWCHAR ) : sizeof( char );
...
void finalize_output_parameters( _Inout_ hdb_stmt* stmt TSRMLS_DC )
{
	...
	ZEND_HASH_FOREACH_KEY_PTR( params_ht, index, key, output_param_temp ) {
		hdb_output_param* output_param = static_cast<hdb_output_param*>( output_param_temp );
        ...
        case IS_STRING:
        {
            ...
            if( str_len == 0 ) {
                core::hdb_zval_stringl( value_z, "", 0 );
                continue;
            }
            ...
            case HDB_ENCODING_BINARY:
                ...
                break;
            default:
                HDB_ASSERT( false, "Invalid encoding in output_param structure." );
                break;
            }
            CHECK_CUSTOM_ERROR( str_len > ( output_param->original_buffer_len - null_size ), stmt,
                HDB_ERROR_OUTPUT_PARAM_TRUNCATED, output_param->param_num + 1 ) {
                throw core::CoreException();
            }
            ...
            if( output_param->encoding != HDB_ENCODING_CHAR && output_param->encoding != HDB_ENCODING_BINARY ) {
				bool converted = convert_zval_string_from_utf16(output_param->encoding, value_z, str_len);
                CHECK_CUSTOM_ERROR( !converted, stmt, HDB_ERROR_OUTPUT_PARAM_ENCODING_TRANSLATE, get_last_error_message()) {
                    throw core::CoreException();
                }
            }
            else if( output_param->encoding == HDB_ENCODING_BINARY && str_len < output_param->original_buffer_len ) {
                ...
                str[ str_len ] = '\0';
                core::hdb_zval_stringl(value_z, str, str_len);
            }
            else {
                core::hdb_zval_stringl(value_z, str, str_len);
            }
        ...
        case IS_DOUBLE:
            ...
            else if (output_param->php_out_type == HDB_PHPTYPE_INT) {
                // first check if its value is out of range
                double dval = Z_DVAL_P(value_z);
                if (dval > INT_MAX || dval < INT_MIN) {
                    CHECK_CUSTOM_ERROR(true, stmt, HDB_ERROR_DOUBLE_CONVERSION_FAILED) {
                        throw core::CoreException();
                    }
                }
            ...
            break;
        default:
            DIE( "Illegal or unknown output parameter type. This should have been caught in core_hdb_bind_parameter." );
            break;
        }
		value_z = NULL;
    }  ZEND_HASH_FOREACH_END();
                          ...
                          _Inout_updates_bytes_(*field_len) void*& field_value, _Inout_ SQLLEN* field_len TSRMLS_DC )
{
    ...
    try {
        ...
        DEBUG_HDB_ASSERT( hdb_php_type.typeinfo.type == HDB_PHPTYPE_STRING,
                             "Type should be HDB_PHPTYPE_STRING in get_field_as_string" );

        if( hdb_php_type.typeinfo.encoding == HDB_ENCODING_DEFAULT ) {
            hdb_php_type.typeinfo.encoding = stmt->conn->encoding();
        }

        // Set the C type and account for null characters at the end of the data.
        switch( hdb_php_type.typeinfo.encoding ) {
        case CP_UTF8:
            c_type = SQL_C_WCHAR;
        ...
        if( sql_display_size == 0 || sql_display_size == INT_MAX ||
                ...
                if( is_truncated_warning( state ) ) {
                    ...
                    if( field_len_temp == SQL_NO_TOTAL ) {
                        ...
                        do {
                            ...
                            field_value_temp = static_cast<char*>( hdb_realloc( field_value_temp, field_len_temp + extra + 1 ));
            ...
            if( hdb_php_type.typeinfo.encoding == HDB_ENCODING_UTF8 ) {

                bool converted = convert_string_from_utf16_inplace( static_cast<HDB_ENCODING>( hdb_php_type.typeinfo.encoding ),
            ...
            field_value_temp = static_cast<char*>( hdb_malloc( sql_display_size + extra + 1 ));
            ...
            if( hdb_php_type.typeinfo.encoding == CP_UTF8 ) {
                                        ...
                                        _Out_writes_(buffer_len) SQLPOINTER& buffer, _Out_ SQLLEN& buffer_len TSRMLS_DC )
{
    ...
    if( buffer_len < expected_len ) {
        HDB_ASSERT( expected_len >= expected_len - buffer_null_extra,
		...
		// buffer when retrieving anything but HDB_ENC_BINARY/SQL_C_BINARY
...
void adjustInputPrecision( _Inout_ zval* param_z, _In_ SQLSMALLINT decimal_digits ) {
    ...
    if( isScientificNot ){
        ...
        HDB_ASSERT( scientificExp <= maxDecimalPrecision, "Input decimal overflow: sql decimal type only supports up to a precision of 38." );
        if( isNegExp ){
...
void save_output_param_for_later( _Inout_ hdb_stmt* stmt, _Inout_ hdb_output_param& param TSRMLS_DC )
{
    ...
    core::hdb_zend_hash_index_update_mem(*stmt, param_ht, paramno, &param, sizeof( hdb_output_param ));
...
// called by Zend for each parameter in the hdb_stmt::output_params hash table when it is cleaned/destroyed
void hdb_output_param_dtor( _Inout_ zval* data )
{
    hdb_output_param *output_param = static_cast<hdb_output_param*>( Z_PTR_P( data ));
    zval_ptr_dtor( output_param->param_z ); // undo the reference to the string we will no longer hold
	hdb_free( output_param );
}

// called by Zend for each stream in the hdb_stmt::param_streams hash table when it is cleaned/destroyed
void hdb_stream_dtor( _Inout_ zval* data )
{
    hdb_stream* stream_encoding = static_cast<hdb_stream*>( Z_PTR_P( data ));
    zval_ptr_dtor( stream_encoding->stream_z ); // undo the reference to the stream we will no longer hold
	hdb_free( stream_encoding );
}
