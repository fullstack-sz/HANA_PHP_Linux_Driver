...
// File: core_util.cpp
//
// Contents: Utility functions used by both connection or statement functions for both the PDO and hdb drivers
// 
...
void core_hdb_register_logger( _In_ log_callback driver_logger )
{
...
{
    HDB_ASSERT( string != NULL, "String must be specified" );
...
bool convert_zval_string_from_utf16( _In_ HDB_ENCODING encoding, _Inout_ zval* value_z, _Inout_ SQLLEN& len)
{
    ...
    bool result = convert_string_from_utf16( encoding, reinterpret_cast<const SQLWCHAR*>(string), int(len / sizeof(SQLWCHAR)), &outString, outLen );
    if( result ) {
       core::hdb_zval_stringl( value_z, outString, outLen );
       hdb_free( outString );
       len = outLen;
    }
    return result;
...
{
    HDB_ASSERT( inString != NULL, "Input string must be specified" );
    HDB_ASSERT( outString != NULL, "Output buffer pointer must be specified" );
    HDB_ASSERT( *outString == NULL, "Output buffer pointer must not be set" );

    if (cchInLen == 0 && inString[0] == L'\0') {
        *outString = reinterpret_cast<char*>( hdb_malloc ( 1 ) );
        *outString[0] = '\0';
        cchOutLen = 0;
        return true;
    }
    ...
    char* newString = reinterpret_cast<char*>( hdb_malloc( cchOutLen + 1 /* NULL char*/ ));
...
SQLWCHAR* utf16_string_from_mbcs_string( _In_ HDB_ENCODING php_encoding, _In_reads_bytes_(mbcs_len) const char* mbcs_string, _In_ unsigned int mbcs_len,
                                        _Out_ unsigned int* utf16_len )
{
    *utf16_len = (mbcs_len + 1);
    SQLWCHAR* utf16_string = reinterpret_cast<SQLWCHAR*>( hdb_malloc( *utf16_len * sizeof( SQLWCHAR )));
    *utf16_len = convert_string_from_default_encoding( php_encoding, mbcs_string, mbcs_len, utf16_string, *utf16_len );

    if( *utf16_len == 0 ) {
        // we preserve the error and reset it because hdb_free resets the last error
        DWORD last_error = GetLastError();
        hdb_free( utf16_string );
                                 ...
                                 TSRMLS_DC )
{
    ...
    HDB_ENCODING enc = ctx.encoding();

    switch( h_type ) {

        case SQL_HANDLE_STMT:
            {
                hdb_stmt* stmt = static_cast<hdb_stmt*>( &ctx );
        ...
        default:
            error = new ( hdb_malloc( sizeof( hdb_error ))) hdb_error();
            ...
            // don't use the CHECK* macros here since it will trigger reentry into the error handling system
            // Workaround for a bug in unixODBC 2.3.4 when connection pooling is enabled (PDO HDB).
...
#ifdef __APPLE__
            if( r == SQL_NO_DATA && ctx.driver() != NULL /*PDO HDB*/ ) {
                r = SQL_SUCCESS;
            }
#endif // __APPLE__
                                      ...
                                      _Out_ hdb_error_auto_ptr& formatted_error, _In_ logging_severity severity TSRMLS_DC, _In_opt_ va_list* args )
{
    ...
    formatted_error->sqlstate = reinterpret_cast<SQLCHAR*>( hdb_malloc( SQL_SQLSTATE_BUFSIZE ));
    formatted_error->native_message = reinterpret_cast<SQLCHAR*>( hdb_malloc( SQL_MAX_ERROR_MESSAGE_LENGTH + 1 ));
...
const char* get_last_error_message( _Inout_ DWORD last_error )
{
    ...
    if( r == 0 ) {
        HDB_STATIC_ASSERT( sizeof( INTERNAL_FORMAT_ERROR ) < sizeof( last_err_msg ));
